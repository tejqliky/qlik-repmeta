...
function labelFor(status?: string | null) {
  ...
  // Preview/under-construction messaging (UI feature not fully shipped yet)
  if (s === "succeeded") return "Preview — Ready";
  if (s === "failed") return "Preview — Failed";
  if (s === "running" || s === "pending") return "Preview — Running";
  // created or unknown
...
export function InsightsChip({ status, onClick }: InsightsChipProps) {
  ...
  return (
    <button
      ...
      title={clickable ? "Preview: AI Insights (in development)" : undefined}
    >
      {s === "running" || s === "pending" ? (
