...
# Force table-based flow representation instead of images
FORCE_TABLE_FLOWS: bool = True

def _add_flow_matrix_table(doc: Document, edges: List[Tuple[str, str, int]], title: str,
                           max_sources: int = 12, max_targets: int = 10):
    """
    Render a Source × Target matrix (task counts). Caps size for readability.
    """
    ...
    # Derive ordered lists of sources/targets by total volume
    from collections import defaultdict as _dd
    s_tot = _dd(int); t_tot = _dd(int)
    for (s, t), n in agg.items():
        s_tot[s] += n
        t_tot[t] += n
    ordered_sources = [s for s, _ in sorted(s_tot.items(), key=lambda kv: (-kv[1], kv[0]))][:max_sources]
    ordered_targets = [t for t, _ in sorted(t_tot.items(), key=lambda kv: (-kv[1], kv[0]))][:max_targets]

    _add_text(doc, title, size=12, bold=True)
    headers: List[str] = ["Source \\ Target"] + ordered_targets + ["Total"]
    rows: List[Tuple[Any, ...]] = []
    ...
    grand_total = 0
    for t in ordered_targets:
        col_sum = sum(agg.get((s, t), 0) for s in ordered_sources)
        total_row.append(_fmt_int(col_sum))
        grand_total += col_sum
    total_row.append(_fmt_int(grand_total))
...
async def generate_customer_report_docx(customer_name: str) -> Tuple[bytes, str]:
    ...
    # Flow Matrix: Customer-level
    _add_flow_matrix_table(doc, customer_edges, "Flow Matrix: Source × Target (Task Counts)", max_sources=12, max_targets=10)
...
# ---- MetricsLog rollups (added) ----
    ...
    for s in [srv["server_name"] for srv in servers]:
        ...
        # Per-server flow matrix
        edges = server_edges_map.get(s, [])
        _add_flow_matrix_table(doc, edges, "Flow Matrix: Source × Target (Task Counts)", max_sources=10, max_targets=8)
