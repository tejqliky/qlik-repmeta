...
async def _endpoint_mix_from_repo(conn, customer_id: int):
    """Compute Endpoint Mix from latest Repository JSON ingest per server (by max run_id).
    Source endpoints that are Log Stream (detected via any 'logstreamstagingtask' key in settings_json
    within any rep_db_%_source table) are counted exclusively under "Log Stream".
    Other sources are counted by canonicalized db_settings_type. Targets are unchanged.
    Returns (source_mix, target_mix) as lists of {"type": str, "uses": int}, sorted (uses desc, type asc).
    """
    import logging, os
    ...
    # 1) Latest run per server for this customer (deterministic by MAX(run_id))
    ...
    dbg("Loaded endpoints rows=%s", len(endpoints))

    if not endpoints:
        dbg("No repo endpoints for customer, returning empty mixes")
        return [], []
    ...
    for r in endpoints:
        if isinstance(r, dict):
            norm.append((int(r.get("endpoint_id")), str(r.get("role") or "").upper().strip(), str(r.get("type") or "").strip()))
        else:
            eid = int(r[0]); role = str(r[1] or "").upper().strip(); typ  = str(r[2] or "").strip()
            norm.append((eid, role, typ))

    # Baseline counts before carve-out (for troubleshooting)
    base_src = {}
    base_tgt = {}
    for (_eid, role, raw) in norm:
        raw = raw or "(unknown)"
        if role == "SOURCE":
            base_src[raw] = base_src.get(raw, 0) + 1
        elif role == "TARGET":
            base_tgt[raw] = base_tgt.get(raw, 0) + 1
    dbg("Baseline SOURCE counts: %s", base_src)
    dbg("Baseline TARGET counts: %s", base_tgt)

    source_eids = [eid for (eid, role, _t) in norm if role == "SOURCE"]
    dbg("Source endpoint_ids count=%s sample=%s", len(source_eids), source_eids[:20])

    # 2) Discover only source-specific tables that actually have settings_json
    tables = await _all(conn, """
        SELECT table_schema, table_name
          FROM information_schema.columns
         WHERE table_schema = %s
           AND column_name  = 'settings_json'
           AND table_name   LIKE 'rep_db\_%%\_source' ESCAPE '\'
    """, (SCHEMA,)) or []
    fq_tables = [f"{t['table_schema']}.{t['table_name']}" for t in tables if isinstance(t, dict) and t.get("table_schema") and t.get("table_name")]
    dbg("Discovered %s source settings tables: %s", len(fq_tables), fq_tables)

    # 3) DB-side detection per-table using UNNEST join (robust)
    logstream_eids = set()
    if source_eids and fq_tables:
        for fq in fq_tables:
            try:
                rows = await _all(conn, f"""
                    WITH e AS (SELECT UNNEST(%s::int[]) AS endpoint_id)
                    SELECT s.endpoint_id
                      FROM {fq} s
                      JOIN e USING(endpoint_id)
                     WHERE CAST(s.settings_json AS text) ILIKE '%%logstreamstagingtask%%'
                """, (source_eids,))
                detected = {int(rr["endpoint_id"] if isinstance(rr, dict) else rr[0]) for rr in (rows or [])}
                dbg("Table %s detected %s logstream endpoints: %s", fq, len(detected), sorted(list(detected))[:50])
                logstream_eids.update(detected)
            except Exception as ex:
                if DBG:
                    log.exception("[LMIX] Table %s detection error", fq)
                continue

    dbg("Total detected Log Stream source endpoint_ids=%s sample=%s", len(logstream_eids), sorted(list(logstream_eids))[:50])

    # 4) Count with carve-out
    src_acc, tgt_acc = {}, {}
    for (eid, role, raw) in norm:
        raw = raw or "(unknown)"
        if role == "SOURCE":
            if eid in logstream_eids:
                label = "Log Stream"
            else:
                label = canonize_to_master(raw, is_source=True) if raw else "Unknown"
            src_acc[label] = src_acc.get(label, 0) + 1
        elif role == "TARGET":
            label = canonize_to_master(raw, is_source=False) if raw else "Unknown"
            tgt_acc[label] = tgt_acc.get(label, 0) + 1

    dbg("POST-CARVE SOURCE counts: %s", src_acc)
    dbg("POST-CARVE TARGET counts: %s", tgt_acc)

    source_mix = [{"type": k, "uses": int(v)} for k, v in src_acc.items()]
    target_mix = [{"type": k, "uses": int(v)} for k, v in tgt_acc.items()]
